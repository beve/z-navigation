...
export default function Text({ children, size = 1, letterSpacing = 0.01, lineHeight = 0.07, color = '#000000', centerX = true, centerY = true, verticalAlign = 'top', ...props }) {
  ...
  const [shapes, [x, y]] = useMemo(() => {
    ...
    return [
      letters.map(letter => {
        ...
        mesh.position.y = lineNum ? ((y+lineHeight)*-lineNum) : y*-lineNum
        return mesh
      }),
      [x, verticalAlign === 'top' ? y : lineNum ? ((y+(lineHeight))*-(lineNum-1)) : y]
    ]
  }, [font, children, size, letterSpacing, color])
  return (
    <group {...props}>
      <group position={[centerX ? -x / 2 : 0, centerY ? -y / 2 : -y, 0]}>
